...
with T[1]:
    ...
    with cc:
        sec("Controls")
        net_ch = st.radio("State", ["Initial — Oct 2023","Final — Aug 2026"],
    ...
    with cm:
        ...
        for nd,nd_d in G_sh.nodes(data=True):
            x,y=pos[nd]; nx_l.append(x); ny_l.append(y); nt_l.append(nd)
        ...
        fig_n.update_layout(height=580, paper_bgcolor="#FFFFFF",
                            plot_bgcolor="#FFFFFF",
...
with T[3]:
    ...
    # manual slider — hidden during playback
    tick_idx = st.slider(
        "Event index",
        min_value=0,
    ...
    # progress bar
    pct = int((st.session_state.tick / max(len(SIM_LOG)-1, 1)) * 100)
    st.markdown(

...
/* radio + checkbox labels — force visible */
...
PB = dict(
    ...
    legend=dict(orientation="h", yanchor="bottom",
                ...
                font=dict(size=11, color="#0A0A0A"))
)
...
def sec(label):
    st.markdown(
        ...
        f'color:#6B6B6B;margin:28px 0 12px 0;padding-bottom:8px;'
        f'border-bottom:1px solid #EEEEEE">{label}</div>',
        unsafe_allow_html=True
    )
...
def bold(txt, color="#0A0A0A", size="13px"):
    return (f'<span style="font-weight:600;font-size:{size};'
...
with T[0]:
    ...
    ir_los = (1 - ir_e/ir_s)*100 if ir_s > 0 else 0
    ...
    m4.metric("Iran Might Loss",    f"{ir_los:.0f}%", f"{ir_s:.3f} → {ir_e:.3f}")
    ...
    for pid, dates, desc in phases:
        ...
        c3.markdown(
            f'<span style="font-size:13px;color:#0A0A0A">{desc}</span>',
            unsafe_allow_html=True
        )
...
with T[1]:
    ...
    with cc:
        ...
        for dot_col, label in [
            ...
            st.markdown(
                f'<div style="font-size:12px;color:#0A0A0A;'
                f'margin-bottom:4px">'
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        for line_col, label in [
            ("#1B7A3E","Friendly tie"),
            ("#C41E3A","Hostile tie"),
        ]:
            st.markdown(
                f'<div style="font-size:12px;color:#0A0A0A;margin-bottom:4px">'
                f'<span style="color:{line_col};font-weight:700;'
                f'margin-right:8px">—</span>{label}</div>',
                unsafe_allow_html=True
            )
        st.markdown(
    ...
    with cm:
        ...
        for nd, nd_d in G_sh.nodes(data=True):
            ...
            nh_l.append(
                f"<b>{nd}</b><br>Might: {m:.3f}"
                f"<br>Alignment: {al:+.3f}<br>Camp: {camp}"
            )
            nc_l.append(
                "#1B3A6B" if al>0.15 else
        ...
        fig_n = go.Figure(data=edge_traces + [ntr])
        fig_n.update_layout(
            height=580,
            paper_bgcolor="#FFFFFF", plot_bgcolor="#FFFFFF",
...
with T[2]:
    ...
    mc2 = (SIM_LOG["top_mediator"].value_counts()
           .reset_index())
    ...
    fig_b = go.Figure(go.Bar(
        ...
        marker=dict(
            color=["#C41E3A" if c==top_c else "#E5E5E5"
...
with T[3]:
    ...
    st.markdown(
        ...
        f'margin-bottom:20px">{snap_ev}</div>',
        unsafe_allow_html=True
    )
    ...
    if len(top3) >= 3:
        ...
        for col, (name, score), border, sz, rank_lbl in configs:
            with col:
                st.markdown(
                    ...
                    f'color:#0A0A0A;text-align:center;line-height:1.2">'
                    f'{name}</div>'
    ...
    st.markdown(
        '<hr style="margin:4px 0 0 0;border:none;'
    ...
    for d, label in key_moments:
        ...
        if not match.empty:
            ...
            rc[1].markdown(
                bold(row["top_mediator"], "#1B3A6B", "13px"),
                unsafe_allow_html=True
            )
            rc[2].markdown(
    ...
    st.markdown(
        ...
        'letter-spacing:0.15em;color:#1B3A6B;margin-bottom:8px">'
        'Key Finding</div>'
        'Our structural model identifies <b>India and China</b> as '
        'best-positioned mediators by network topology — large states '
        'maintaining non-hostile ties to both camps. Real-world mediation '
        'was carried out by <b>Qatar</b> (2023 hostage deal, 2025 Gaza '
        'ceasefire), <b>Oman</b> (US–Iran back-channel, Strait of Hormuz '
        'MOU), and <b>Pakistan</b> (Apr 2026 ceasefire, Jun 2026 14-point '
        'MOU). This divergence shows structural position is '
        '<em>necessary but not sufficient</em> for mediation — pointing '
        'to model extensions incorporating political trust and '
    ...
    with col_mod:
        ...
        for rank, (name, score) in enumerate(
            ...
            st.markdown(
                ...
                f'color:#6B6B6B">{score:.4f}</span></div>'
    ...
    with col_real:
        ...
        for name, roles in [
            ...
            st.markdown(
                ...
                f'color:#0A0A0A;margin-bottom:3px">{name}</div>'
                f'<div style="font-size:12px;color:#6B6B6B;'
                f'line-height:1.6">{roles}</div></div>',
                unsafe_allow_html=True
            )
...
with T[4]:
    ...
    with f3: srch   = st.text_input("Filter by actor",
    ...
    if srch:
        mask = (
            filt["actor1"].str.contains(srch, case=False, na=False) |
    ...
    fig_g.update_layout(**PB, height=220,
    ...
    for hc, hl in zip(hcols2,
                      ["Date","Actor 1","Actor 2","Description","Score"]):
        hc.markdown(
            f'<span style="font-size:10px;font-weight:700;'
            f'text-transform:uppercase;letter-spacing:0.1em;'
            f'color:#6B6B6B">{hl}</span>',
    ...
    for _, row in filt.iterrows():
        ...
        rc[1].markdown(
            ...
            f'color:#0A0A0A">{row["actor1"]}</span>',
            unsafe_allow_html=True
        )
        rc[2].markdown(
            f'<span style="font-weight:500;font-size:12px;'
            f'color:#0A0A0A">{row["actor2"]}</span>',
            unsafe_allow_html=True
        )
        rc[3].markdown(
